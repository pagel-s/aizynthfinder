...
def test_get_actions_two_policies_limits_ringbreaker_to_ring_molecules(
    default_config, setup_template_expansion_policy, mocker
):
    expansion_policy = default_config.expansion_policy
    uspto_strategy, _ = setup_template_expansion_policy("uspto")
    ringbreaker_strategy, _ = setup_template_expansion_policy("ringbreaker")
    expansion_policy.load(uspto_strategy)
    expansion_policy.load(ringbreaker_strategy)
    expansion_policy.select(["uspto", "ringbreaker"])
    mols = [
        TreeMolecule(smiles="c1ccccc1", parent=None),
        TreeMolecule(smiles="CCO", parent=None),
    ]
    cache_molecules = [
        TreeMolecule(smiles="Oc1ccccc1", parent=None),
        TreeMolecule(smiles="CCCO", parent=None),
    ]
    seen = {}

    def make_actions(policy_name, molecules):
        actions = []
        priors = []
        for mol in molecules:
            actions.extend(
                [
                    SmilesBasedRetroReaction(
                        mol,
                        reactants_str=f"{mol.smiles}.O",
                        metadata={"policy_name": policy_name},
                    ),
                    SmilesBasedRetroReaction(
                        mol,
                        reactants_str=f"{mol.smiles}.N",
                        metadata={"policy_name": policy_name},
                    ),
                ]
            )
            priors.extend([0.7, 0.2])
        return actions, priors

    def wrapped_uspto_get_actions(molecules, cache_molecules=None):
        return make_actions("uspto", molecules)

    def wrapped_ringbreaker_get_actions(molecules, cache_molecules=None):
        seen["molecules"] = [mol.smiles for mol in molecules]
        seen["cache_molecules"] = [
            mol.smiles for mol in (cache_molecules or [])
        ]
        return make_actions("ringbreaker", molecules)

    mocker.patch.object(uspto_strategy, "get_actions", side_effect=wrapped_uspto_get_actions)
    mocker.patch.object(
        ringbreaker_strategy, "get_actions", side_effect=wrapped_ringbreaker_get_actions
    )

    actions, priors = expansion_policy.get_actions(mols, cache_molecules)

    assert seen["molecules"] == ["c1ccccc1"]
    assert seen["cache_molecules"] == ["Oc1ccccc1"]
    policy_names = [action.metadata["policy_name"] for action in actions]
    assert policy_names == ["uspto"] * 4 + ["ringbreaker"] * 2
    assert priors == [0.7, 0.2, 0.7, 0.2, 0.7, 0.2]
